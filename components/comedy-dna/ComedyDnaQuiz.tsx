...
const SHOW_URL = (slug: string) => `${SITE_URL}/shows/${slug}/`;
...
export default function ComedyDnaQuiz({ quiz, fingerprints, comingSoon = [], jokeCount = 0 }: { quiz: QuizData; fingerprints: ShowFingerprint[]; comingSoon?: { slug: string; name: string }[]; jokeCount?: number }) {
  ...
  const shareThisPick = useCallback((e: React.MouseEvent, winner: QuizJoke, loser: QuizJoke) => {
    ...
    const txt = `I picked "${trim(winner.text)}" over "${trim(loser.text)}" — which one would you pick? ${SITE_URL}/comedy-dna`;
    const nav = typeof navigator !== 'undefined' ? navigator : null;
    if (nav?.share) {
      nav.share({ text: txt, url: `${SITE_URL}/comedy-dna` }).catch(() => { /* user dismissed */ });
          ...
          {epRecs.length > 0 && (
            <div className="mt-8">
              ...
              <div className="bg-brand-card border border-brand-border rounded-2xl px-5 py-1">
                {epRecs.map(ep => (
                  <a key={`${ep.slug}-${ep.s}-${ep.e}`} href={`${SITE_URL}/shows/${ep.slug}/${ep.s}/${ep.e}/`} target="_blank" rel="noopener noreferrer"
